...
/// Header: 1 byte tag + 2 bytes channels.
...
pub fn audio_to_bytes(channels: u16, samples: &[f32], buf: &mut [u8]) -> usize {
    let count = samples.len().min(MAX_SAMPLES_PER_PACKET);
    buf[0] = TAG_AUDIO;
    buf[1..3].copy_from_slice(&channels.to_be_bytes());
...
pub fn audio_from_bytes(buf: &[u8]) -> Option<(u16, &[u8])> {
    if buf.len() < HEADER_SIZE || buf[0] != TAG_AUDIO {
        return None;
    }
    let channels = u16::from_be_bytes([buf[1], buf[2]]);
    Some((channels, &buf[HEADER_SIZE..]))
}
...
mod tests {
    ...
    fn roundtrip_audio() {
        ...
        let len = audio_to_bytes(2, &samples, &mut buf);
        let (channels, raw) = audio_from_bytes(&buf[..len]).unwrap();
